@extends('layout.app')
@section('title', 'Edit Postingan — Violet')

@section('content')
    <div class="sticky top-0 z-20 glass border-b border-modern px-4 py-3 flex items-center gap-6">
        <a href="{{ route('posts.show', $post->id) }}" class="text-blue-400 hover:text-blue-300 text-sm font-semibold flex items-center gap-1">
    ...
    <div class="p-6">
        <form method="POST" action="{{ route('posts.update', $post->id) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            @endif
            @if($post->file_path)
                <p class="text-xs text-gray-600">📎 File saat ini: {{ $post->file_name }}</p>
            @endif
