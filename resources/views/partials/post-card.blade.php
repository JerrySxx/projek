<div class="flex gap-3 p-4 border-border-modern hover-modern transition-all duration-200">
    ...
    <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 text-[15px]">
            ...
            <span class="text-gray-500 truncate text-sm">{{ $post->user->username }} ‌ {{ $post->created_at->diffForHumans() }}</span>
        </div>
        ...
        <a href="{{ route('posts.show', $post->id) }}">
            <img src="{{ Storage::url($post->image_path) }}" class="mt-3 rounded-2xl border border-modern w-full object-cover max-h-[400px] ring-1 ring-white/5" alt="">
        </a>
        @endif
        @if($post->file_path)
        <a href="{{ Storage::url($post->file_path) }}" download="{{ $post->file_name }}" class="inline-flex items-center gap-2 mt-3 px-3 py-2 bg-white/5 border border-modern rounded-xl text-blue-400 text-sm hover:bg-white/10 transition-colors">
            <i data-lucide="file-text" class="w-4 h-4"></i> {{ $post->file_name }}
        </a>
        @endif
        <div class="flex items-center gap-1 mt-3 -ml-2">
           ...
           <button type="button"
        onclick="event.stopPropagation(); togglePostLike(this, '{{ $post->id }}');"
        class="flex items-center gap-1.5 text-sm transition-colors p-2 rounded-full hover:bg-pink-500/10 {{ $post->isLikedByAuthUser() ? 'text-pink-500' : 'text-gray-500 hover:text-pink-400' }}">
